...
export const GoalProvider = (props) => {
    ...
    useEffect(() => { // once the component is mounted - executes getAllGoals only when necessary
        async function fetchData() { // fetches goal data
            await getAllGoals();
        }
        fetchData();
    }, []);

    function getAllGoals() { // retrieve all goals
        return axios.get(baseUrl).then(response => setGoal(response.data));
    }

    function getGoal(id) { // retrieve goal by id
